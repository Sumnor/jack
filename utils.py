...
async def load_active_war_rooms():
    ...
    try:
        records = supabase.select("war_rooms")
        active_war_rooms = {}
...
async def save_war_room_to_db(war_id: str, war_room_data: Dict):
    ...
    try:
        data = {
            ...
            "guild_id": war_room_data.get("guild_id"),
            "channel_id": war_room_data.get("channel_id"),
        ...
        filters = {"war_id": str(war_id).strip()}
        try:
            supabase.update("war_rooms", data, filters)
        except:
            supabase.insert("war_rooms", data)
...
async def delete_war_room_from_db(war_id: str):
    ...
    try:
        supabase.delete("war_rooms", {"war_id": str(war_id).strip()})
