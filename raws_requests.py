...
from settings_multi import get_auto_requests_sheet
from request_x import parse_amount
        ...
        for record in records:
            try:
                ...
                for res in total_week:
                    ...
                    total_week[res] += per_day * 5  # 5 days per week
...
async def auto_resources_for_prod_req(
    ...
    try:
        # Check if user already has an auto request
...
async def disable_auto_request(interaction: discord.Interaction):
    ...
    try:
        # Check if user has any auto requests
        existing = supabase.select('auto_requests', filters={'guild_id': guild_id, 'discord_id': user_id})
