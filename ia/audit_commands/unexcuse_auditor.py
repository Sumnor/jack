...
@bot.tree.command(name="unexcuse_auditor", description="Remove excuse status of an auditor")
...
async def excuse_auditor(interaction: discord.Interaction, auditor_id: str):
    ...
    supabase.update("auditor_quotas", {"excused": False}, {"guild_id": interaction.guild_id, "auditor_id": int(auditor_id)})
    await interaction.response.send_message(
            f"✅ Successfully **unexcused** <@{auditor_id}> from their current quota.",
            ephemeral=True
        )