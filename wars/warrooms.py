...
async def find_existing_war_room(guild: discord.Guild, enemy_id: str, category_id: int) -> Optional[discord.TextChannel]:
    """Find an existing war room for the same enemy"""
    try:
...
async def add_member_to_war_room(
    ...
    """Add a member to an existing war room"""
    try:
        ...
        for existing_war_id, war_data in active_war_rooms.items():
            if war_data.get('channel_id') == channel.id:
                ...
                # Also track this new war_id mapping to the same channel
        ...
        print(f"Error adding member to war room: {e}")
        return False


async def update_war_room_description(channel: discord.TextChannel, war_ids: List[str]):
    """Update channel topic to show all war IDs"""
    try:
        topic = f"Active Wars: {', '.join(war_ids)}"
        await channel.edit(topic=topic[:1024])  # Discord has 1024 char limit
    except Exception as e:
...
async def create_war_room(
    ...
    """Create a war room or add member to existing room"""
    try:
        ...
        if existing_room:
            print(f"Found existing war room {existing_room.name} for enemy {enemy_id}")
            ...
            # Update description with all war IDs
        ...
        # Include enemy_id in channel name for easy lookup
...
async def update_war_room_access(guild: discord.Guild, war_id: str, current_participants: Set[str]):
    """Update permissions for war room participants"""
    try:
        ...
        # Check if any wars are still active in this room
        ...
        if not current_participants or not wars_in_room:
            ...
            # Delete all war_id entries pointing to this channel
...
async def run_node_listener():
    ...
    try:
        ...
        # dynamically set node_modules path
...
async def handle_pnw_events():
    """Main event handler for PnW events"""
